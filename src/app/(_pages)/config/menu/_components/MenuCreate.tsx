...
import { createMenu } from "../_libs/action";
...
export default function MenuCreate(props: Props) {
  const { isOpen, onClose, menuGroupData } = props;
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (confirm("Submit this data?")) {
      ...
      try {
        const result = await createMenu(formData as any);
        if (result.status) {
          setAlertModal({
            status: true,
            color: "success",
