...
type Props = {
  ...
  subDepartmentEdit: SubDepartmentProps;
};

export default function SubDepartmentEdit(props: Props) {
  const { isOpen, onClose, departmentData, subDepartmentEdit } = props;
  ...
  const [formData, setFormData] = useState({
    id: subDepartmentEdit.id || (null as number | null),
    department: subDepartmentEdit.department?.id || (null as number | null),
    nama_sub_department: subDepartmentEdit.nama_sub_department || "",
    leader: subDepartmentEdit.leader_user?.id || (null as number | null),
    supervisor:
      subDepartmentEdit.supervisor_user?.id || (null as number | null),
    manager: subDepartmentEdit.manager_user?.id || (null as number | null),
    akses_izin:
      subDepartmentEdit.akses_izin?.map((e) => ({
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (confirm("Submit this data?")) {
      ...
      try {
        const result = await editSubDepartment(formData as any);
        if (result.status) {
          setAlertModal({
            status: true,
            color: "success",
