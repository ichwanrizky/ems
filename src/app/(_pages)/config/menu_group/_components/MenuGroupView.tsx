...
import { isLoadingProps, MenuGroupProps } from "@/types";
...
export default function MenuGroupView() {
  ...
  const [menuGroupEdit, setMenuGroupEdit] = useState({} as MenuGroupProps);
  ...
  const handleGetEdit = async (id: number) => {
    ...
    try {
      const result = await getMenuGroupId(id);
      if (result.status) {
        setMenuGroupEdit(result.data as MenuGroupProps);
        setIsEditOpen(true);
      } else {
        setAlertPage({
          status: true,
  ...
  const handleDelete = async (id: number) => {
    if (confirm("Delete this data?")) {
      setIsLoadingAction({ ...isLoadingAction, [id]: true });
      try {
        const result = await deleteMenuGroup(id);
        if (result.status) {
          setAlertPage({
            status: true,
            color: "success",
