...
import { editUserLuar } from "../_libs/action";
...
export default function UserLuarEdit(props: Props) {
  const { isOpen, onClose, userLuarEdit } = props;
  ...
  const [formData, setFormData] = useState({
    id: userLuarEdit?.id || (0 as number),
    name: userLuarEdit?.name || ("" as string),
    telp: userLuarEdit?.telp || ("" as string),
    role_id: userLuarEdit?.roles.id || (null as number | null),
    username: userLuarEdit?.username || ("" as string),
    new_password: "" as string,
    re_new_password: "" as string,
  });
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (confirm("Submit this data?")) {
      ...
      try {
        const result = await editUserLuar(formData as any);
        if (result.status) {
          setAlertModal({
            status: true,
            color: "success",
